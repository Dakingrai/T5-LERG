...
from target_models import GPT, T5
...
def process_data(data):
    r_data = []
    for line in data:
        r_data.append((line["input"], line["predictSQL"]))
    return r_data
...
def explain_dataset(explainer, model_f, tokenizer, data_path):
    ...
    if not os.path.exists("exp/"):
        os.mkdir("exp")
    for x, y in tqdm.tqdm(r_data):
        # experiment on sentences with length less than 30, such that can get explanation using 8G GPU
        if len(tokenizer.tokenize(x)) <= 300 and len(tokenizer.tokenize(y)) <= 300:
...
if __name__ == "__main__":
    ...
    model = T5(model_dir=args.model_dir)
