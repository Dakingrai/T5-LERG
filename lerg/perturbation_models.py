...
class RandomPM(BasicPM):
    ...
    def perturb_inputs(self, x, num=1000, with_i=None, question_only=False):
        """
        allow half tokens at most can be replaced
        """
        if question_only:
            question_end_index = x.index("▁|")
            dist, num_comb = binomial_coef_dist(len(x[:question_end_index]))
        else:
        ...
        for _ in range(num):
            ...
            if with_i is None:
                ...
                if question_only:
                    indices_to_repl = list(range(len(x)))[:question_end_index]
                else:
