...
class LERG_SHAP_log(LERG):
    ...
    def get_local_exp(self):
        ...
        self.y = self.tokenizer.tokenize(self.y)
        phi_sets = []
        for_loop = copy.deepcopy(self.x) 
        if question_only:
            question_end_index = self.x.index("▁|")
            indices_to_repl = list(range(len(self.x)))[:question_end_index]
            for_loop = copy.deepcopy(indices_to_repl)
        for i in range(len(for_loop)):
            x_set, x_set_with_i, weights, self.components = \
                self.perturb_inputs(self.x, num=500//len(self.x), with_i=i, question_only=question_only)# results in total 1000 samples as LERG_LIME
